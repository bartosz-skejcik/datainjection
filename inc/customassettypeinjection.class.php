...
/**
 * Custom Asset Type injection class for GLPI v11 custom asset definitions.
 * This class dynamically handles injection for custom asset type dropdowns.
 */
class PluginDatainjectionCustomAssetTypeInjection extends AssetType implements PluginDatainjectionInjectionInterface
{
    /**
    ...
    public static function getDefinition(): AssetDefinition
    {
        ...
        if (!($definition instanceof AssetDefinition)) {
            throw new \RuntimeException('Asset definition is expected to be defined for custom asset type injection.');
        }
        return $definition;
    }
    ...
    public function getOptions($primary_type = '')
    {
        ...
        // Get search options from the concrete type class
    ...
    public function addOrUpdateObject($values = [], $options = [])
    {
        ...
        // Create an instance of the concrete type class
        $type = new $type_class();
        ...
        $lib = new PluginDatainjectionCommonInjectionLib($type, $values, $options);
...
/**
 * Factory class to create type injection instances for specific custom asset definitions.
 */
class PluginDatainjectionCustomAssetTypeInjectionFactory
{
    /**
    ...
    /**
     * Create a dynamic injection class for a custom asset type
     */
    private static function createInjectionClass(AssetDefinition $definition): void
    {
        $system_name = $definition->fields['system_name'];
        $class_name = 'PluginDatainjection' . $system_name . 'AssetTypeInjection';
        $type_class = $definition->getAssetTypeClassName();
        $cache_key = $system_name . '_type';

        // Create the class dynamically
        $class_code = <<<PHP
class {$class_name} extends {$type_class} implements PluginDatainjectionInjectionInterface
{
    public static function getTable(\$classname = null)
    {
...
PHP;
    ...
    /**
     * Get all injectable custom asset type types
     */
